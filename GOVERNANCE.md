...
A normative change that landed under the 14-day window without an
explicit statement from an author is not thereby agreed for submission.
Before a revision is submitted, each normative change made since the
previous revision carries an explicit yes from each author, named by
commit. Lazy consensus governs the repository; it does not accumulate
into a submission.
