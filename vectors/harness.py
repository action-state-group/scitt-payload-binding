...
def _reference_impl_main() -> int:
    ...
    try:
        # UTF-8 explicitly: sys.stdin.read() decodes with the locale codec, so a
        # C/POSIX locale turns every non-ASCII vector into surrogates before any
        # rule can look at it. The protocol pins UTF-8 in both directions.
...
def _run_command(command: str, vector_json: str) -> tuple[int, str, str]:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            # Pin the codec. Everything upstream is scrupulous about bytes --
            # read_text(encoding="utf-8"), the bytes-on-disk check, the raw-span
            # slicing -- and then text=True alone would encode stdin with the
            # locale's preferred encoding, mangling non-ASCII vectors on any
            # machine whose locale is not UTF-8. The last hop has to be pinned too.
            encoding="utf-8",
            timeout=30,
        )
...
def verify_impl(command: str, root: Path) -> int:
    ...
    for vec_path in sorted(root.rglob("*.json")):
        ...
        # Not an assert: python -O strips those, and everything below depends on
        # raw_text being the file's exact bytes.
        if vec_path.read_bytes() != raw_text.encode("utf-8"):
            raise ValueError(f"bytes-on-disk mismatch: {vec_path}")
...
# A conforming implementation of algorithm jcs-n, suitable for use as the
# external command in `python3 vectors/harness.py verify-impl` and as a starting
# point for a port. It passes the suite as generated -- if you change it and the
# suite goes red, the change is the thing to look at.
#
...
# Wire rules (CANONICALIZATION_DECLARATION.md sections 2 and 2a). These live at the
# token layer: by the time json.loads has built a dict, "-0" is already 0 and a
# duplicate key has already collapsed, so they must be enforced during the parse.
_WIRE_INT = re.compile(r"^(0|-?[1-9][0-9]*)$")

def _int_or_die(s):
    if not _WIRE_INT.match(s): raise ValueError(f"non-integer number token: {s!r}")
    return int(s)

def _no_floats(s): raise ValueError(f"floating-point token in a digest-bearing field: {s!r}")
...
def run():
    raw = sys.stdin.buffer.read().decode("utf-8")
    try:
        json.loads(raw, parse_int=_int_or_die, parse_float=_no_floats,
...
is written to the command's stdin as UTF-8, and its stdout is read as UTF-8.
The command MUST decode stdin as UTF-8 explicitly rather than relying on the
locale: a C/POSIX locale otherwise turns every non-ASCII vector into
replacement characters or surrogates before any rule can examine it, which
turns a conformance question into an environment question.  The command MUST:
